...
class Handler {
  ...
  async SendScore(message, sender, sendResponse) {
    ...
    chrome.storage.local.get(["logined_account"], function (result) {
      if (JSON.stringify(result) == "{}") result["logined_account"] = {};
      if (result["logined_account"][message.ccxpAccount] != json_hash) {
