...
class Handler {
  ...
  async SendScore(message, sender, sendResponse) {
    ...
    let PostScoreAndStorage = (result) => {
      result["existed_hash"].push(json_hash);
    ...
    //Data format ==> result = {"existed_hash" :[hash_1, hash_2]}
    //Data format ==> result = {"logined_account" :{"account": "hash","aaa": "1234"}};

    chrome.storage.local.get(["existed_hash"], function (result) {
      if (JSON.stringify(result) == "{}" ) {
        result= {"existed_hash":[]};
        PostScoreAndStorage(result);
        console.log("空",result);
      }else if (result["existed_hash"].find(e => e == json_hash) != json_hash) {
        PostScoreAndStorage(result);
        console.log("新增",result);
      }else {
        console.log("沒變",result);
      }
    });
  }
